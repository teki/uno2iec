	...
	, m_simulatedState(simsOff)
{
	...
	readSettings();
	m_port.setBaudRate(static_cast<qint32>(m_appSettings.baudRate));
...
void MainWindow::enumerateComPorts()
{
	...
	static QSerialPortInfo piPort = { "/dev/ttyAMA0", "ttyAMA0", "Arduino AMA0", "", 0, 0 };
	m_ports.insert(0, piPort);
	//	m_port.setBaudRate(BAUD57600/*BAUD1152000*/);
#elif defined(Q_OS_LINUX)
	static QSerialPortInfo unixPort = { "/dev/ttyACM0", "ttyACM0", "Arduino ACM0", "", 0, 0 };
	...
	foreach(const QFileInfo entry, dev.entryInfoList()) {
		static QSerialPortInfo unixPort = { entry.absoluteFilePath(), entry.fileName(), entry.fileName(), "", 0, 0 };
		m_ports.insert(0, unixPort);
	}
#endif
} // enumerateComPorts
